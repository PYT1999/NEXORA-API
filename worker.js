...
// NEXORA 34.1.3 API — Cloudflare Worker + D1 + Realtime Realms + Password Recovery
// D1 binding: env.DB
...
async function resolveSecretSafe(binding,label){
  try{
    return await resolveSecret(binding);
  }catch(e){
    console.error(`NEXORA secret binding failed: ${label}`,e);
    return "";
  }
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (
        ...
        // Absichtlich immer gleiche öffentliche Antwort,
        // damit nicht erkannt werden kann, ob ein Account existiert.
        const publicResult={
          ok:true,
          message:
      ...
      if (
        ...
        // Alle offenen Reset-Codes ungültig machen.
        await env.DB
          .prepare(`
            UPDATE password_reset_codes
            SET used_at=?
        ...
        // Genau eine neue Session erzeugen.
        const session=
          await createSession(
            env.DB,
            user.id
      ...
      // ==================================================
      // AUTHENTICATED CHARACTER / SAVE ROUTES
      // ==================================================
      ...
      if (
        ...
        const used=
          await env.DB
            .prepare(
              "SELECT slot FROM characters WHERE user_id=? ORDER BY slot"
            )
            .bind(auth.id)
            .all();
      ...
      await fetch(
        "https://api.resend.com/emails",
        {
          ...
          body:JSON.stringify({
            ...
            from:
              "NEXORA <noreply@mail.nexorasystems.ch>",
...
Falls du das nicht angefordert hast, ignoriere diese E-Mail.`,
            ...
            html:
              `<div style="font-family:Arial,sans-serif;background:#0b0d10;color:#eee;padding:28px">
                <h2 style="letter-spacing:.12em">NEXORA</h2>

                <p>
                  Du hast das Zurücksetzen deines Passworts angefordert.
                </p>

                <p style="font-size:30px;font-weight:800;letter-spacing:.25em">
                  ${code}
                </p>
...
function safeSocketAttachment(ws){
...
function publicRealmPlayer(player){
